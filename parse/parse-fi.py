#!/usr/bin/env python3
"""
finnish wordnet parser for orbihex
extract finnish words and build semantic lexicon
"""
...
class finnish_wordnet_parser:
    def __init__(self, data_file='omw-data/wns/fin/wn-data-fin.tab'):
        self.data_file = data_file
        self.words = set()
        
    def parse_finnish_wordnet(self):
        """parse finnish wordnet data"""
        if not os.path.exists(self.data_file):
            print(f"finnish wordnet not found: {self.data_file}")
            return []
        
        print(f"parsing finnish wordnet from {self.data_file}...")
        ...
        with open(self.data_file, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f):
                ...
                # clean word: remove spaces, special chars, keep finnish letters
        ...
        print(f"total finnish words: {len(self.words)}")
        return self.words
    
    def save_finnish_words(self, filename='dataset/fi/words.txt'):
        """save finnish word list"""
        with open(filename, 'w', encoding='utf-8') as f:
            for word in self.words:
                f.write(f"{word}\n")
        print(f"saved {len(self.words)} finnish words to {filename}")

class finnish_lexicon:
    def __init__(self, words, output_file='lexicon/fi/fi.pkl'):
        self.words = words
    ...
    def build_finnish_lexicon(self):
        """build finnish lexicon"""
        print(f"building finnish lexicon for {len(self.words)} words...")
        ...
        print(f"finnish lexicon built: {len(self.lexicon)} words")
        return self.lexicon
    
    def save_lexicon(self):
        """save finnish lexicon"""
        with open(self.output_file, 'wb') as f:
            pickle.dump(self.lexicon, f)
        
        file_size = os.path.getsize(self.output_file)
        print(f"saved finnish lexicon to {self.output_file} ({file_size:,} bytes)")
...
def main():
    """main finnish wordnet processing"""
    print("=== finnish wordnet orbihex processing ===")
    
    # create directories
    os.makedirs('dataset/fi', exist_ok=True)
    os.makedirs('lexicon/fi', exist_ok=True)
    
    # parse finnish wordnet
    parser = finnish_wordnet_parser()
    finnish_words = parser.parse_finnish_wordnet()
    parser.save_finnish_words()
    
    # build finnish lexicon
    builder = finnish_lexicon(finnish_words)
    finnish_lexicon_data = builder.build_finnish_lexicon()
    ...
    print(f"lexicon size: {len(finnish_lexicon_data)} words")
    print(f"compression ratio: {compression_ratio:.1f}x")
    print(f"mass range: {analysis['mass_range'][0]:.3f} - {analysis['mass_range'][1]:.3f}")
    
    return finnish_lexicon_data, analysis

if __name__ == "__main__":
    finnish_lexicon, analysis = main()
